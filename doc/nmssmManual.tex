...
measurement, one can augment the MSSM by a gauge singlet chiral
superfield~\cite{Ross:2011xv,Ross:2012nr}. We shall call this model the
next-to-minimal 
supersymmetric standard model (NMSSM), but shall distinguish between a
version where an extra symmetry is assumed (often a $\mathbb{Z}_3$ symmetry) 
and a version where it is not ($\Zv$).
...
supersymmetry. In
But
next-to-minimal helps naturalness. NMSSMTools and SPheno with SARAH.
