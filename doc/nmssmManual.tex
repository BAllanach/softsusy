...
doublet superfield \red{$H_1$} %%% 
coupling to the down-type quarks have the same 
...
\end{align}
%
\noindent where \red{$(Y_{U,D,E})_{ij}$ and $\lambda,\kappa$ are dimensionless Yukawa couplings, $\mu$ and $\mu'$ are supersymmetric mass terms, and $\xi_F$ encodes the effects of the supersymmetric tadpole term.  We use} %%% 
the subscript $\Zv$~\red{to} %%%
reflect the fact that this superpotential 
...
\label{eq:mins}
\end{align}
\red{where $M_Z^2 = \tfrac{1}{4}\bar{g}^2(v_1^2+v_2^2)$ and $\overline{g} = (g_2^2+g^{\prime 2})^{1/2}$ 
...
\ee 
%
The mass matrix \red{$M^2_{H^0}$ is obtained by expanding $H_{1,2}$ and $S$ about their VEVs (\ref{eq:potmin}) and identifying terms $-(H^0)^T M^2_{H^0} H^0$ in the Lagrangian.  Typically, the resulting matrix elements $(M_{H^0}^2)_{ij}$ are simplified by using the tree-level 
electroweak symmetry breaking (EWSB) conditions (\ref{eq:mind}-\ref{eq:mins}) in order to eliminate the soft terms $m_{H_1}^2$, $m_{H_2}^2$ and $m_S^2$.} %%% 
% \be
% M_{H^0}^2=
% \left(
% \begin{array}{ccc}
...
- \ds\frac{\delta_{ij}}{v_i}\ds\frac{\partial V}{\partial v_i} \qquad \red{\mbox{with } v_3\equiv s\,,}
\ee
%
\red{and under this prescription we find} %%%   
%
\ba
...
charged Goldstone boson \red{$G^{\pm}$} %%% 
and a charged Higgs, $H^\pm$ with mass
%
\be 
...
The electroweak symmetry breaking (EWSB) conditions \red{(\ref{eq:mind}-\ref{eq:mins}) 
...
the MSSM.  By} %%%
including tadpole corrections $t_i$ and the transverse self energy $\Pi^T_{ZZ}$ of the 
$Z$ boson, \red{we find} %%% 
%
\begin{align}
...
in \cite{Pierce:1997zz}.  We use the same procedure in the NMSSM, with \red{expressions 
...
Note that} %%%
this ensures a simple MSSM limit for threshold corrections. 
...
\red{(If the iteration does not converge to the desired accuracy, \SOFTSUSY~outputs a 
...
%% \subsection{SLHA interface for NMSSM-\SOFTSUSY}
%% \label{sec:appendix-nmssm-slha-interface}
...
\code{sPhysical} \red{structure} %%% 
had to be generalized, as in
