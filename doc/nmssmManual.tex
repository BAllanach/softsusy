...
\SOFTSUSY~convention to evolve $\bar DR$ couplings to $M_Z$ as the final step,
...
\end{verbatim}
%
The \code{setup\_nmssmtools.sh} script copies \code{nmh\_slhainp.f}
...
\code{NMSSMTools\_4.1.2/main/}. The file \code{decay}
contains the sparticle decays in form of SLHA DECAY blocks,
\code{omega} will contain the output from \code{micrOMEGAS} if entry 16 is selected to be non-zero and
\code{spectr} contains the spectrum calculated by
\NMSSMTools.
