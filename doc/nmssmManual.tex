...
We now describe the algorithm used to perform the calculation.  The full iterative algorithm to determine the mass \red{spectrum} %%%
 is shown schematically in Fig.\ \ref{fig:algorithm}.  Here we will provide a detailed description of this procedure and specify all contributions that are included in \red{the calculation.} %%%  

As in MSSM \red{\SOFTSUSY, the Standard Model fermion and gauge boson masses, 
and the couplings $\alpha(M_Z)$, $G_F^\mu$, and $\alpha_s(M_z)$ act as low-energy constraints. 
 Below $M_Z$, the evolution of these input parameters} %%%
 proceeds \red{in the manner} %%%
 described in \red{Sec.\ 3.1} %%%
 of the MSSM \red{\SOFTSUSY} %%%
 manual \cite{Allanach:2001kg}.  \red{Similarly,} %%%
the initial guess for the SUSY preserving $\overline{DR}$ parameters at $m_t$ 
follows the procedure \red{outlined in Sec.\ 3.2 of \cite{Allanach:2001kg},} %%% 
with \red{the additional} %%%
 NMSSM parameters $\{\lambda, \kappa, s, \xi_F, \mu^\prime \}$ either initially set to their \red{(user specified)} %%% 
input values, or to zero in the case \red{when $\kappa$ and $s$ are treated as} %%%
 output from the EWSB conditions \red{(Sec.\ \ref{ewsb}).} %%%      
...
separately.)  Such a procedure has been carried out by EHT, who make use of the 
...
found agreement with the results of EHT once the third family approximation 
(\ref{eq:3rd fam}) is enforced.  Note that in the \SOFTSUSY\ conventions, all 
$\beta$ functions are real.

We also incorporate the two-loop running for $\tan\beta$ and the Higgs vacuum 
expectation values (VEVs) $v_{1,2}$ and $s$.  Here, we make use of the results 
obtained by Sperling et al.\ \cite{Sper13}, where the purely NMSSM contributions
