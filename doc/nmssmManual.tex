...
\ba 
 W^{general}_{NMSSM} &=&\hat{\bar U} \, \mathbf{y_u} \hat{Q} \hat{H}_u 
-\hat{\bar D} \, \mathbf{y_d} \hat{Q} \hat{H}_d 
-\hat{\bar E} \, \mathbf{y_e} \hat{L} \hat{H}_d 
+(\lambda \hat{S} + \mu)(\hat{H}_u \hat{H}_d)+\frac{1}{3}\kappa\hat{S}^3 + \zeta_F\hat{S} + \frac{1}{2} \mu^\prime\hat{S}^2 \\
 &=& W_{MSSM}^{\mu =0}  +(\lambda \hat{S} + \mu)(\hat{H}_u \hat{H}_d)+\frac{1}{3}\kappa\hat{S}^3 + \zeta_F\hat{S} + \frac{1}{2} \mu^\prime\hat{S}^2 
. 
\label{eq:gensuperpotential}
\ea
 
Imposing the $Z_3$ symmetry restricts the superpotential to,

\ba 
 W_{NMSSM} &=& \hat{\bar U} \, \mathbf{y_u} \hat{Q} \hat{H}_u 
-\hat{\bar D} \, \mathbf{y_d} \hat{Q} \hat{H}_d 
-\hat{\bar E} \, \mathbf{y_e} \hat{L} \hat{H}_d 
+\lambda \hat{S}(\hat{H}_u \hat{H}_d)+\frac{1}{3}\kappa\hat{S}^3 \\
 &=& W_{MSSM}^{\mu =0}  +\lambda \hat{S} (\hat{H}_u \hat{H}_d)+\frac{1}{3}\kappa\hat{S}^3.
\label{eq:superpotential}
\ea

In this form the NMSSM superpotential contains no explicit mass parameter, thereby solving the $\mu-problem$
