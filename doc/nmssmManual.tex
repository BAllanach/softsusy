...
From these results, it is possible to derive the explicit expressions in a chosen model, e.g.\ the work of Martin and Vaughn (MV) \cite{MV94} provides a complete list of the RGEs for the MSSM with inter-generational quark mixing.  

Overlap with MSSM.

In this appendix, we present the NMSSM contributions with inter-generational mixing.  In each case, we have checked that our results reduce to the results of EHT in the limit where only the third generation couplings are non-vanishing.
...
obtained by identifying indices in (\ref{eq:Yuk rges}) with the relevant chiral superfields in the superpotential.  For example, for $k=H_2$ we have 
$Y^{ijH_2}\equiv (Y_U)^{ij}$.  
