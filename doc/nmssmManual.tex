...
\end{align}
%
Here $C_a(k)$ is the quadratic Casimir invariant for chiral superfield $\Phi_k$,
 with complex conjugation on downstairs indices understood: 
$Y_{ijk} \equiv Y_{ijk}^*$.  Note that the Yukawa and trilinear couplings are 
...
\cite{MV94,Yam94} for $Y_{U,D,E}$ is the inclusion of $\lamsq$ terms which 
...
$G_a$.  To obtain a non-zero contribution from $\lambda$ we must have  
$k\neq S_k$ (since $C_a(S)=0$) and furthermore $C_3(H_k)=0$.  The result is
%
\begin{equation}
...
tracing over $SU(2)$ group indices.

For completeness, we reproduce the MSSM parts as well
%
\be
\dt g_a = \frac{g_a^3}{16\pi^2}B_a^{(1)} + \frac{g_a^3}{(16\pi^2)^2}\Bigg[\sum_{b=1}^3 B_{ab}^{(2)}g_b^2 - \sum_{x=U,D,E}C_a^x\tr(Y_xY_x^\dagger) - \lamsq \Lambda_a^{(2)}\Bigg]\,,
\ee
%
where $B_a^{(1)} = (33/5,1,-3)$, and 
%
\be
B_{ab}^{(2)} = \left(\begin{array}{ccc}
199/5 & 27/5 & 88/5 \\
9/5   & 25   & 24   \\
11/5  & 9    & 14   \end{array}\right) \quad \mbox{and} \quad
C_a^{U,D,E} = \left(\begin{array}{ccc}
26/5 & 14/5 & 18/5 \\
6    & 6    & 2    \\
4    & 4    & 0    \end{array}\right) \quad \mbox{and} \quad 
\Lambda_a^{(2)} = (6/5,2,0)\,.
\ee
...
\end{equation}
%
where $U^{ijk}$ is the trilinear soft SUSY-breaking parameter. The full expression reads
% 
\be
\dt M_a = \frac{2g_a^2}{16\pi^2}(B_a^{(1)} - \lamsq\Lambda^{(2)}_a)M_a + \frac{2g_a^2}{(16\pi^2)^2}\Bigg\{\sum_{b=1}^3B_{ab}^{(2)}g_b^2(M_a+M_b) + \sum_{x=U,D,E}C_a^x[\tr(U_x^\dagger Y_x) - M_a\tr(Y_xY_x^\dagger)] + \lambda a_\lambda \Lambda_a^{(2)}\Bigg\}
\ee

\subsubsection{$\mu$ Parameters}
The general expression for the SUSY-conserving bilinear terms \cite{MV94,Yam94} is given by  
%
\begin{equation}
