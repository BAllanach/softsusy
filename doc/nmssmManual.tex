...
Here, we present the one- and two-loop renormalization group equations (RGEs) in the $\overline{\mbox{DR}}$ scheme for parameters of the $\Zv$ NMSSM.  Our strategy is to make use of existing results \cite{MV94,Yam94} for a general, semi-simple SUSY gauge theory.   

Where possible, we refer the reader to the MSSM results --- saves space.

Herein, we follow the conventions laid out in the SOFTSUSY manual for the MSSM.  hence those adopted by Martin and Vaughn (MV) \cite{MV94}.  There is one important difference: with respect to the MV notation, the Yukawas couplings $Y_{U,D,E}$ and soft SUSY-breaking trilinears $U_A,D_A,E_A$ are {\it transposed}, as in the {\tt SOFTSUSY} code. The complete set of two-loop renormalization group equations (RGEs) for the parameters of the MSSM can be found in \cite{MV94,Yam94}.  For the NMSSM, \cite{Ellwanger:2009dp} (EHT) list the two-loop RGEs in the third generation approximation, where the results for the soft SUSY-breaking terms (and others) originate from \cite{Ell08}.

\subsubsection{Yukawa Couplings}
For $t = \ln Q$, the Yukawa couplings evolve according to the general expression
 \cite{MV94}
%
\begin{equation}
...
\end{equation}
%
and $\gamma^{(1,2)j}_i$ are the one- and two-loop anomalous dimensions respectively.  Note that the Yukawa (and trilinear couplings) are 
written with the same ordering as fields in the superpotential (\ref{eq:WZ3V}), 
e.g.\ for $k = H_2$ we get
%
\begin{equation}
\dt (Y_U)^{ij} = (Y_U)^{ij}\Gamma^{H_2}_{H_2} + (Y_U)^{jp}\Gamma_{Q_p}^{Q_i} 
+ (Y_U)^{ip}\Gamma_{U_p}^{U_j}\,.
\end{equation}

In the one-loop approximation, the only modification to the MSSM results 
\cite{MV94} for $Y_{U,D,E}$ is the inclusion of $\lamsq$ terms which 
originate from the Higgs anomalous dimensions
%
\begin{align}
...
\end{align}
%
At two-loop order, all the gauge-Yukawa contributions from $\lambda$ cancel for each 
$\gamma_i^{(2)j}$, so the additional contributions arising in the NMSSM are 
simply given by
%
\begin{align}
...
For the additional Yuwaka couplings $\lambda$ and $\kappa$, the RGEs read
%
\begin{align}
...
\end{equation}
%
where $U^{ijk}$ is the trilinear soft SUSY-breaking parameter. Evaluating the summations in (\ref{eqn:dM}), we find
%
\begin{equation}
\left. \dt M_a\right|_{\lambda} = \frac{2g_a^2}{(16\pi^2)^2} (\lambda a_\lambda - \lambda^2 M_a)\Lambda^{(2)}_a\,.
\end{equation}

\subsubsection{$\mu$ Parameters}
...
\subsubsection{Higgs Masses}
To determine the $\lambda$ and $\kappa$ contributions to the Higgs masses, it's useful to define the following quantities
%
\begin{align}
...
\end{align}
%
Both the up- and down-type Higgs masses $m_{H_2},m_{H_1}$ receive the same $\lambda$ contribution at {\bf 1-loop}, so consider (say)
%
\begin{align}
\beta_{\mhusq}^{(1)} &= Y_{H_2pq}Y^{pqH_2}\mhusq + 2Y_{H_2pq}Y^{H_2pr}(m^2)_r^q + h_{H_2pq}h^{H_2pq}\,.
\end{align}
%
Restricting myself to just the $\lambda$ contributions, the first sum can be found in (\ref{eqn:YH2sq}) while the other two are given by
%
\begin{align}
Y_{H_2pq}Y^{H_2pr}(m^2)_r^q =& \lamsq\mssq + \lamsq\mhdsq + 3\tr(\mqsq Y_u Y_u^\dagger) 
+ 3\tr(  Y_u\musq Y_u^\dagger) \,, \\
%
h_{H_2pq}h^{H_2pq} =& 2a_\lambda^2 + 6\tr( U_A U_A^\dagger) \,.
\end{align}
%
Thus the result for either $m_{H_1}$ or $m_{H_2}$ is 
%
\begin{equation}
\left.\beta_{m_{H_\alpha}^2}^{(1)}\right|_\lambda = 2\lamsq \mlamsq\,.
\end{equation}
%

The {\bf 2-loop} expressions are rather involved; My working for the up-type Higgs mass beta function will be included in an Appendix (eventually!) and the down-type calculation follows similarly.  By collecting all the terms, I get:
%
\begin{align}
...
\end{align}
%
Similarly, I obtain for the down-type Higgs,
%
\begin{align}
...
For the singlet mass $m_S$, the {\bf 1-loop} result is
%
\begin{equation}
...
\subsubsection{Squark and Slepton Masses}
...
\end{equation}
%
For the soft term $\xi_S$, I had to dig out the general RGE from \cite{Yam94} because MV don't include the tadpole as part of $\mathcal{L}_{\mathrm{soft}}$.  I'll define the general soft tadpole term as $t^i\phi_i$, for which the RGE reads
%
\begin{equation}
\dt t^i = \frac{1}{16\pi^2} \beta_{t^i}^{(1)} + \frac{1}{(16\pi^2)^2} \beta_{t^i}^{(2)}\,,
\end{equation}
%
where the {\bf 1-loop} $\beta$ function is given by\footnote{Note that on pg.~43 of Martin's primer \cite{Mar97}, the expression for $\beta_{t^i}^{(1)}$ is incomplete.}
%
\begin{align}
\beta_{t^i}^{(1)} &= \tfrac{1}{2}Y^{ijk}Y_{jkl}t^l + h^{ijk}Y_{jkl}L^l + \mu^{il}Y_{ljk}b^{jk}
+ 2Y^{ijk}(m^2)_k^l\mu_{jl} + h^{ijk}b_{jk}\,.
\end{align}
%
For $i=S$, $t^S \equiv \xi^{}_S$, and thus I get
%
\begin{align}
\beta_{\xi^{}_S}^{(1)} &= \tfrac{1}{2}Y^{Sjk}Y_{jkS}\xi^{}_S + h^{Sjk}Y_{jkS}\xi^{}_F + \mu'Y_{Sjk}b^{jk} \notag \\
%
&+ 2Y^{Sjk}(m^2)_k^l\mu_{jl} + h^{Sjk}b_{jk}\,.
\end{align}
%
The various summations yield
%
\begin{align}
Y^{Sjk}Y_{jkS} &= 4\lamsq + 4\kapsq\,, \\
%
h^{Sjk}Y_{jkS} &= 4\lambda a_\lambda + 4\kappa a_\kappa\,, \\
%
Y_{Sjk}b^{jk} &= 4\lambda m_3^2 + 2\kappa m_S'^2\,, \\
%
Y^{Sjk}(m^2)_k^l\mu_{jl} &= 2\lambda\mu (\mhusq + \mhdsq) + 2\kappa\mu' \mssq\, \notag \\
%
h^{Sjk}b_{jk} &= 4a_\lambda m_3^2 + 2a_\kappa m_S'^2\,,
\end{align}
%
noting that the terms involving $\msprsq$ carry a factor of $\tfrac{1}{2}$ due to the differing normalisations between soft mass terms for $S$ and those for $H_\alpha$ ({\it viz.} $b_{SS} = \tfrac{1}{2}\msprsq = b^{SS}$).  At {\bf 2-loop} I obtain
%
\begin{align}
...
Here I list the $\lambda$ and $\kappa$ contributions to the RGEs for the scalar masses $\mtrisq \equiv B\mu$ and $\msprsq \equiv B'\mu'$.  For the former, the relevant terms for the {\bf 1-loop} $\beta$ function are
%
\begin{align}
\beta_{\mtrisq}^{(1)} &\ni \tfrac{1}{2}\mtrisq Y_{H_2mn}Y^{mnH_2} + \tfrac{1}{2}\lambda Y_{Smn}b^{mn} + \mu Y_{H_2mn}h^{mnH_2} 
+ (H_2 \leftrightarrow H_1)\,.
\end{align}
%
There are no new sums involved here, and thus the final result is
%
\begin{equation}
\left.\beta_{\mtrisq}^{(1)}\right|_\lambda = 2\lambda(3\lambda\mtrisq + 2\mu a_\lambda) + 2\lambda\kappa\msprsq\,,
\end{equation}
%
which agrees with EHT.  At {\bf 2-loop} I obtain
%
\begin{align}
...
For $\msprsq$, the {\bf 1-loop} $\beta$ function doesn't involve any new sums, however one must keep track of a factor of 2 on the left-hand side of the RGE: $d_t b^{SS} = \tfrac{1}{2}d_t\msprsq$, and hence the 1- and 2-loop $\beta$ functions must be multiplied by 2. The result is:
%
\begin{align}
\beta_{\msprsq}^{(1)} &= 2\big\{\tfrac{1}{2}\msprsq Y_{Smn}Y^{mnS} + \kappa Y_{Smn}b^{mn} + 2\mu' Y_{Smn}h^{mnS}\big\} \notag \\
%
&= 4\lambda(\lambda\msprsq + 2\mu' a_\lambda) + 8\kappa(\kappa\msprsq + \mu' a_\kappa) + 8\lambda\kappa\mtrisq \,.
\end{align}
%
%
At {\bf 2-loop} I get
