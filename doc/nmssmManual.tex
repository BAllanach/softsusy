...
+(\lambda \hat{S} + \mu)(\hat{H}^a_2 \hat{H}^b_1) \right ] +\frac{1}{3}\kappa\hat{S}^3 + \red{\xi}_F\hat{S} + \frac{1}{2} \mu^\prime\hat{S}^2 \\
 &=& W_{MSSM}^{\mu =0}  +  \epsilon_{ab}\left [(\lambda \hat{S} + \mu)(\hat{H}^a_2 \hat{H}^b_1) \right ]+\frac{1}{3}\kappa\hat{S}^3 + \red{\xi}_F\hat{S} + \frac{1}{2} \mu^\prime\hat{S}^2. 
\label{eq:WZ3V}
\ea
...
For a general, semi-simple SUSY gauge theory, the expressions \cite{MV94,Yam94} for the one- and two-loop renormalization group equations (RGEs) in the $\overline{\mbox{DR}}$ scheme are known.  In particular, the RGEs for the MSSM with inter-generational quark mixing are listed by MV.  Here, we derive the NMSSM contributions with inter-generational mixing.  In each case, we have checked that our results reduce to the results of EHT in the limit where only the third generation couplings are non-vanishing.

The relevant pieces which arise in the NMSSM can be deduced by simply appending to the MSSM the additional contributions from superpotential parameters and soft susy-breaking sector.

Here, we present the one- and two-loop renormalization group equations (RGEs) in the $\overline{\mbox{DR}}$ scheme for parameters exclusive to the $\Zv$ NMSSM.  Our strategy is to make use of existing results \cite{MV94,Yam94} for a general, semi-simple SUSY gauge theory.   

In the interest of brevity, we refer the reader to the MSSM results collected in \cite{MV94}.

Results in the thrid family approximation

We have checked that our results reproduce those of \cite{Ellwanger:2009dp} in the third family approximation.

Have general mixing.

Herein, we follow the conventions laid out in the SOFTSUSY manual for the MSSM.  hence those adopted by Martin and Vaughn (MV) \cite{MV94}.  There is one important difference: with respect to the MV notation, the Yukawa couplings $Y_{U,D,E}$ and soft SUSY-breaking trilinears $U_A,D_A,E_A$ are {\it transposed}, as in the {\tt SOFTSUSY} code. The complete set of two-loop renormalization group equations (RGEs) for the parameters of the MSSM can be found in \cite{MV94}.  For the NMSSM, \cite{Ellwanger:2009dp} (EHT) list the two-loop RGEs in the third generation approximation, where the results for the soft SUSY-breaking terms (and others) originate from \cite{Ell08}.
...
respectively.  Evidently, the $3\times 3$ Yukawa matrices $Y_{U,D,E}$ are obtained by fixing a single index, e.g.\ for $k=H_2$ we have $Y^{ijH_2}\equiv (Y_U)^{ij}$.  

At one-loop order, the only addition to the MSSM expressions 
\cite{MV94} for the $Y_{U,D,E}$ RGEs is the inclusion of $\lamsq$ terms which 
...
For the Yuwaka couplings $\lambda$ and $\kappa$, the RGEs read
%
\begin{align}
...
To determine the $\lambda$ and $\kappa$ contributions to the Higgs masses, it is useful to define the following quantities \cite{Ellwanger:2009dp}
%
\begin{align}
...
\end{align}
%
Both the up- and down-type Higgs masses $m_{H_2},m_{H_1}$ receive the same $\lambda$ contribution at one-loop order, 
%
\begin{equation}
...
The two-loop expressions for the up-type Higgs are 
%
\begin{align}
...
\end{align}
%
Similarly, we obtain for the down-type Higgs,
%
\begin{align}
...
Here we list the $\lambda$ and $\kappa$ contributions to the RGEs for the scalar masses $\mtrisq \equiv B\mu$ and $\msprsq \equiv B'\mu'$.  At one-loop we get
%
\begin{equation}
