...
\ee
%
with $v_3 \equiv s$.  The explicit \red{matrix elements read} %%%   
%
\ba
...
\label{eq:MH0}
\ea
\red{where $\overline{g} = (g_2^2+g^{\prime 2})^{1/2}$ for gauge couplings $g_2$ and $g^{\prime}=\sqrt{3/5}g_1$ of $SU(2)_L$ and (unnormalised) 
...
in \cite{Pierce:1997zz}.  We use the same procedure in the NMSSM, with self energies 
generalised to NMSSM and using the NMSSM the lightest Higgs mass and the coupling 
which will match that of the MSSM when in the MSSM limit.  This ensures a simple 
MSSM limit for threshold corrections. 
