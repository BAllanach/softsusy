...
\ref{sec:appendix-nmssm-slha-interface}.
...
NMSSM-\SOFTSUSY\ accepts input in the SLHA-2 format
\cite{Allanach:2008qq}.  The set of input parameters which also exist
in the MSSM are entered as described in \cite{Skands:2003cj}, just as
for the MSSM version of \SOFTSUSY, while the new NMSSM parameters are
all given in the \code{EXTPAR} block as outlined in
\cite{Allanach:2008qq}.  For example one can specify:
%
\begin{verbatim}
Block EXTPAR
   23   0                    # mu
...
Instead of chosing the above parameter sets as EWSB output, it is also
