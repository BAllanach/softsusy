...
\address{Albert Einstein Center for Fundamental Physics, Institute for Theoretical Physics, University of Bern, Sidlerstrasse 5, CH-3012 Bern, Switzerland}
\address{ARC Centre of Excellence for Particle Physics at 
the Tera-scale, School of Chemistry and Physics, University of Adelaide, 
Adelaide SA 5005 Australia}
...
\begin{abstract}
  ...
  \SOFTSUSY~program to calculate the sparticle spectrum in the
  ...
  the minimal supersymmetric standard model fields. Often, a $\mathbb{Z}_3$
  symmetry is 
  imposed upon the model. \SOFTSUSY~can calculate the spectrum in this
  case as well as the case where general $\mathbb{Z}_3$ violating terms are
  added to 
...
TeV-scale supersymmetric particles have so far not been found\footnote{In some
  cases, lower   bounds of 1 
TeV or more have been placed upon gluinos and squarks by LHC experiments.}
at the LHC~\cite{Aad:2013wta,CMSspart}, but searches for them continue, as does
theoretical interest in supersymmetric models. 
This is a
testament to the 
theoretical successes of weak-scale supersymmetry: chiefly the resolution of
the technical hierarchy problem (but also because it contains a convenient
dark matter candidate). 
In order to do supersymmetric phenomenology, a long calculational chain is
...
Higgs'. Currently, in the minimal supersymmetric standard model (MSSM), there
...
level~\cite{Aad:2012tfa,Chatrchyan:2012ufa} whose properties are consistent
with a Standard Model Higgs boson. Using 4.8 fb$^{-1}$ of 7 TeV data and 20.7
...
way to the Standard Model Higgs boson. At tree-level, it's mass is bounded by
...
requiring some level (at the level of one in several tens) of cancellation 
...
measurement~\cite{Delgado:2010uj,Ellwanger:2011mu,King:2012tr,Perelstein:2012qg}, one can augment the MSSM by a gauge singlet chiral
superfield~\cite{BasteroGil:2000bw,Ellwanger:2009dp}. We
shall call this model the 
next-to-minimal 
supersymmetric standard model (NMSSM)~\cite{NMSSM}, but shall distinguish
between a 
version where an extra symmetry is assumed (often a $\mathbb{Z}_3$ symmetry) 
and a version where it is not
($\Zv$)~\cite{Delgado:2010uj,Ross:2011xv,Ross:2012nr}. 
In the MSSM, (based on a two Higgs doublet version of the Standard Model with
...
cancellations. This (along with several other factors) is leading to a lot of
investigation of the NMSSM in the 
recent literature, and so it is important for the research community that we
have reliable computational tools to calculate the relevant observables. 

As mentioned above in the MSSM case, the initial step in this chain is
...
marry {\tt SARAH}~\cite{Staub:2010jh,Staub:2012pb,Staub:2013tta} with {\tt
  SPheno}~\cite{Porod:2003um} in order to be 
able to 
calculate the spectrum after setting the model up. The NMSSM was included in
...
{\tt NMSDECAY}~\cite{Das:2011dg,Muhlleitner:2003vg} calculates sparticle
decays. \code{PYTHIA}~\cite{Sjostrand:2007gs} is then capable of simulating
particle collisions in the NMSSM and micrOMEGAs~\cite{Belanger:2008sj}
...
generalisations of the MSSM. Some are easier (or harder) to use for certain
assumptions about supersymmetry breaking mediation, for instance. 
We strongly suspect that the advantages of having
several publicly available, supported spectrum generators will extend to the
case of the NMSSM, which is receiving so much attention in the literature.
We see the extension of the well known {\tt SOFTSUSY}~program to include the
NMSSM as a beneficial step for the community that will hopefully aid the
accuracy and feasibility of various NMSSM studies. 

In the present paper, we focus on the recent pieces that have been added to
...
code in the {\tt doc/}~subdirectory. The other manuals detail the standard
...
The rest of the paper proceeds as follows: in section~\ref{sec:notation}, we
introduce the NMSSM supersymmetric and the soft supersymmetry breaking
parameters in our conventions.  In section~\ref{sec:calculation}, we describe
the algorithm 
...
\ref{sec:run}, we state how to run the program. The class structure,
along with the data contained within each class, is shown in
\ref{sec:objects}. Finally, in \ref{sec:RGEs}, we reproduce
...
Standard Model (SM) gauge quantum numbers. We denote an $SU(3)$ colour 
...
%
\begin{align} 
 W_{\slashed{\mathbb{Z}}_3}  &=  \epsilon_{ab} \left[ (Y_E)_{ij} L_i^b H_1^a \bar{E}_{j} 
...
contains terms which violate the $\mathbb{Z}_3$ symmetry which is commonly 
...
\end{align}
%
\noindent The $\mathbb{Z}_3$-NMSSM superpotential (\ref{eq:WZ3C}) contains no 
...
(\ref{eq:WZ3C}) and \Zv-NMSSM for the general $\mathbb{Z}_3$ violating one 
(\ref{eq:WZ3V}). 
...
found in Table 1 of MSSM \SOFTSUSY~manual \cite{Allanach:2001kg}.  Elsewhere, 
...
and thus consistent with the review of Ellwanger, Hugonie and Teixeira (EHT) 
 \cite{Ellwanger:2009dp} and ref.~\cite{Degrassi:2009yq}. (Note however that 
our definitions of the neutral Higgs VEVs (Sect.~\ref{sec:hpot}) differ by a 
factor of $\sqrt{2}$ compared to \cite{Ellwanger:2009dp,Degrassi:2009yq}.)


%%% LCT: Commented out the following in case we reinstate the table.
% In Table~\ref{tab:conv} we provide a comparison of the 
% \SOFTSUSY~conventions with the SUSY Les Houches Accord~\cite{Allanach:2008qq}, ref.~\cite{MV94} (Martin and Vaughn), and ref.~\cite{Degrassi:2009yq} (Degrassi and Slavich).
...
+ \frac{\kappa A_\kappa}{3} S^3 + V_{\slashed{\mathbb{Z}}_3} \,,
\ee 
%
where all \Zv~terms are included in
\be 
V_{\slashed{\mathbb{Z}}_3} =  \xi_S S + \frac{m_S^{\prime \, 2}}{2} S^2
...
$m_3^2$ present in $V_2$ is set to zero to avoid double counting with the third term in (\ref{eq:VZ3V}).
...
\ee
%
with $v_3 \equiv s$.  The explicit matrix elements are  
%
\ba
...
Therefore the CP-odd mixing can be stored as a single mixing angle.%
  \footnote{\SOFTSUSY~does this internally by storing $\theta_{A^0}$ in the 
    {\tt sPhysical} object (see \ref{nmssmsoftsusy}).  Note that the SLHA output
...
As in MSSM \SOFTSUSY, the Standard Model fermion and gauge boson masses, and the
...
treated as outputs from the EWSB conditions (Sect.\ \ref{ewsb}).
...
family approximation (\ref{eq:3rd fam}) is enforced.  Note that in the 
...
are reproduced in \ref{sec:RGEs}.
...
$M_{SUSY}$ where $M_Z^2$ and $\tan\beta$ are predicted as part of a consistency 
...
include a test at $M_{SUSY}$ to determine whether the chosen parameter space 
point corresponds to a global minimum (as done in the {\tt NMSSMTOOLS} CHECKMIN
...
offering helpful remarks.  LCT is supported by the Federal Commission for Scholarships for Foreign Students.
...
In \SOFTSUSY\ 4 the command line interface of \code{softpoint.x} has
...
\normalsize
%
where only \code{sugra} is the currently available susy-breaking
...
\SOFTSUSY~4 has a compatibility mode which interfaces with NMSSMTools
4.1 to calculate sparticle decays in the NMSSM.  To enable it, the
...
main/ directory within the NMSSMTools folder.  If the \code{--compile}
flag is provided, NMSSMTools is recompiled.  Afterwards the user can
generate an NMSSM spectrum with \SOFTSUSY\ and use NMSSMTools to
calculate the decays.  The \code{run\_nmssmtools.sh} script combines
these two steps:
%
\begin{verbatim}
  $ ./run_nmssmtools.sh slhaInput slhaOutput
\end{verbatim}
%
Here \code{slhaInput} is an SLHA input file with the SOFTSUSY block entry $15$ set to $1$.  \code{slhaOutput} is the name of the SLHA output file which will contain the spectrum and the decays. Additional NMSSMTools specific flags can also be used with entries $16$ and $17$, which are past to NMSSMTools as MODSEL blocks $9$ and $10$ respectively, following the NMSSMTools convention.

%
\begin{verbatim}
   Block SOFTSUSY
      15   1      # NMSSMTools compatible output (default: 0) 
      16   4      # Selecty Micromegas option for NMSSMTools
...
\end{verbatim}
%
After \code{run\_nmssmtools.sh} is called, the following three output
...
\code{nmProcessSpec-omega} will contain the output from micromegas if entry 16 is selected to be non-zero and
