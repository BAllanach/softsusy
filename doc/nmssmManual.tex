...
\item The class of the soft breaking MSSM parameters and their beta
  functions was converted into a class template \code{SoftPars<Susy>}.
...
The NMSSM spectrum calculation class \code{NmssmSoftsusy} inherits
from the \code{Softsusy<SoftPars>} template class, where the template
parameter is set to \code{SoftParsNmssm}.  \code{NmssmSoftsusy} then
overwrites some MSSM functions to account for the extra NMSSM
particles.
