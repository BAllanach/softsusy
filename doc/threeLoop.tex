...
the difference in bottom and tau Yukawa couplings doesn't change much due to
the higher order corrections: from 6-9$\%$ or so. 
However, bottom-top Yukawa unification is made slightly better, by about a
percent or so (seen from Figs~\ref{fig:cmssmUn}e,\ref{fig:cmssmUn}f), but at
$\tan \beta=30$, this does not work so well {\bf CHANGE tanb}.
...
\caption{\label{fig:sig} Relative effect of highest order terms (three-loop
  ...
  parameters coincide with the latest ATLAS searches for jets and missing
  energy interpreted in the 
  CMSSM~\cite{ATLAS-CONF-2013-047,Aad:2013wta}. The region below the dashed line is excluded at the 95$\%$
  confidence level by at least one of the most restrictive ATLAS jets plus
  missing energy searches.} 
\end{figure}
The reduction of gluino and squark masses change the SUSY
production cross-section, making it larger. 
As Fig.~\ref{fig:sig} shows, this results in an increase between 10-26$\%$ in
the region not excluded by current searches. This is therefore our estimate
for the theoretical uncertainty upon the NLO cross-section induced from
spectrum uncertainties (note however that it does not include theoretical
uncertainties coming from the NNLO cross-section, however that can easily be
obtained from scale dependence in the NLO result).
