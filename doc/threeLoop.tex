...
are not far below the GUT scale, for example $M_{GUT}/10$~{\bf CITE}. 
...
In the following, we shall examine two parameter points in detail
...
\begin{equation}
\Delta
\alpha=\alpha_3(M_{GUT})-\alpha_1(M_{GUT}),\
\Delta Y=Y_b(M_{GUT})-Y_\tau(M_{GUT}),\ 
\Delta Y'=Y_t(M_{GUT})-Y_b(M_{GUT}).
\end{equation}
...
\caption{\label{tab:pmssm}  Differences due to the highest order terms
  ...
  families. 
  The rows marked with a $\Delta$ show the {\em change} in
  the mass from the {\tt SOFTSUSY3.4.1} result except for 
  the columns labeled $\Delta \alpha$, $\Delta Y$ or $\Delta Y'$. The column
  labeled `RGEs' 
...
couplings. Such a discrepancy may be explained within a more detail GUT model
from GUT threshold effects, but the precise value of
