...
excluded if=1
 ...
 14   <tryToConvergeHard> # (Default=0), will go slow in attempt to get solution
\end{verbatim}
\normalsize
...
\code{displayAltEwsb} \\ 
& conditions & \\ \hline
\code{double t1OV1Ms} & 2-loop tadpole $t_1/v_1(M_S)$ & \code{displayTadpole1Ms} \\
\code{double t2OV2Ms} & 2-loop tadpole $t_2/v_2(M_S)$ & \code{displayTadpole2Ms} \\
\code{double t1OV1Ms1loop} & 1-loop tadpole $t_1/v_1(M_S)$ & \code{displayTadpole1Ms1loop} \\
\code{double t2OV2Ms1loop} & 1-loop tadpole $t_2/v_2(M_S)$ & \code{displayTadpole2Ms1loop} \\
\hline
