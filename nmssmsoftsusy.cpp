...
//Version for general non-z_3 invariant nmssm, 
//in case of Z3 invariance outputs s instead of mu 
int NmssmSoftsusy::rewsbMu(int sgnMu, double & mu, double & s, bool Z3) const {
  int flag = 0;
   if (abs(sgnMu) != 1) {
    ostringstream ii;     
  ...
  //but now \musqeff = \mu^2 + 0.5 lambda^2 s^2
...
//PA:  general nmssm version.  In case of Z3 invariance 
// outputs kappa instead.
int NmssmSoftsusy::rewsbM3sq(double mu, double & m3sq, double & kap, bool Z3) const {
  int flag = 0;
  ...
  if (fabs(mu) < 1.0e-9)
     ...
     double  m3sqeff  =  0.5 * 
        (displayMh1Squared() + displayMh2Squared() - displayTadpole2Ms() -
         displayTadpole1Ms() + 2.0 * sqr(mu) + sqr(lam) * sqr(s)) * 
        sin(2 * atan(displayTanb()));
     
     if (Z3) {
        //PA this doesn't work when lam = 0.  Should add warning.
        kap = (2.0 * m3sqeff / s - sqrt(2.0) * alam ) / (s * lam);
     }
     else{
        kap = displayKappa(); 
...
double NmssmSoftsusy::predTanb() const  {
...
//PA: nmssm couplings to be added.
double NmssmSoftsusy::doCalcTadpole1oneLoop(double mt, double sinthDRbar) {
 ...
 if (forLoops.mu(1, 3) == 0.0 || forLoops.mu(2, 3) == 0.0) {
  ...
  //PA: new NMSSM parameters required.
  double svev = displaySvev();
  double lam = displayLambda();
  /// sneutrino coupling
  //PA:  No NMSSM modification to this, since
  //NMSSM has no right handed snuetrino N to couple 
  // like y_N N L H_d
  double lSnu = g * mz / costhDRbar * 0.5 * cosb;
  
  /// stop couplings
  //PA: This is modified by new F-term part from 
  //\lambda * s * y_t which mirrors the \mu * y_t term 
  DoubleMatrix lTS1Lr(2, 2), lTS112(2, 2), rotate(2, 2);
  lTS1Lr(1, 1) = g * mz / costhDRbar * guL * cosb;
  lTS1Lr(1, 2) = ht * mu / root2; 
  lTS1Lr(1, 2) +=  0.5 * ht * lam * svev; //NMSSM modification
  ...
  //PA if we ignore left-right mass mixing 
  // there are no new NMSSM modifications 
...
void NmssmSoftsusy::calcTadpole1Ms1loop(double mt, double sinthDRbar) { /// checked
...
double NmssmSoftsusy::displayMwRun() const {
...
void NmssmSoftsusy::calcTadpole2Ms1loop(double mt, double sinthDRbar) {/// CHECKED
...
double NmssmSoftsusy::predMzsq(double & tanb) const {
  tanb = predTanb(); 
...
double nmftCalc(double x) {
  ...
  if (ftFunctionality <= ftPars.displayEnd()) {
    ...
    ostringstream ii;
    ii << "NmssmSoftsusy:nmftCalc called with incorrect functionality=" <<
...
double NmssmSoftsusy::it1par(int numPar, const DoubleVector & bcPars) {
  ...
  nmsPhysical savePhys(displayPhys());
  ...
  if (fabs(x) < 1.0e-10) {
    ...
    derivative = calcDerivative(nmftCalc, x, h, &err);
...
(void (*boundaryCondition)(NmssmSoftsusy &, const DoubleVector &),
 const DoubleVector  & bcPars, double mx, bool doTop) {
  ...
  nmftBoundaryCondition = boundaryCondition;
...
void NmssmSoftsusy::iterateMu(double & muold, int sgnMu,
  ...
  //PA: this will later have to be an input to function
  bool Z3 = false;
  double lam = displayLambda();
  double s = displaySvev();
  ...
  try {
    ...
    if(Z3){ 
       ...
       //PA: fill munew so it can be fed into rewsbM3sq.
       // Could also remove this as a parameter from rewsbM3sq
       // and simply set and get mu
       munew = displaySusyMu();
    }
    
    else{     
    double oneLoopMusq = oneLoopMusqeff - 0.5 * sqr(lam) * sqr(s);
    /// Error in rewsb
     if (oneLoopMusq < 0.0) {
          err = 2; 
    ...
    //PA:  using a rewsbM3sq which can work for Z3 symmetric version
    //with kappa as output
    //but nonetheless setting Z3=false and not using this yet  
    if (rewsbM3sq(munew, m3sqnew, kapnew, Z3) == 0) {
       flagM3sq(false);
    }
    else{
       flagM3sq(true);
...
void NmssmSoftsusy::rewsbTreeLevel(int sgnMu) {
  ...
  bool Z3 = false;
   if (rewsbMu(sgnMu, mu, s, Z3)) flagMusqwrongsign(true);
   else flagMusqwrongsign(false);
  setSusyMu(mu);
  //PA:  again using a rewsbM3sq which can work for Z3 symmetric version
  //with kappa as output
  //but nonetheless setting Z3=false and not using this yet  
  if (rewsbM3sq(mu, m3sq, kap, Z3)) flagM3sq(true);  
  else flagM3sq(false); 
...
void NmssmSoftsusy::rewsb(int sgnMu, double mt, double muOld) {
  ...
  //PA:  again using a rewsbM3sq hich can work for Z3 symmetric version
  //with kappa as output
  //but nonetheless setting Z3=false and not using this yet  
  if (rewsbM3sq(munew, m3sqnew, kapnew, Z3) == 0) flagM3sq(false);
  else flagM3sq(true); 
...
void NmssmSoftsusy::printShort() const {
  ...
  int pos;
  nmsPhysical s(this->displayPhys());
...
void NmssmSoftsusy::printLong() {
  ...
  if (displayProblem().tachyon) {
    NMdrBarPars s(displayDrBarPars());
    s.me = -1.0 * s.me;
    s.md = -1.0 * s.md;
    s.mu = -1.0 * s.mu;
    s.msnu = -1.0 * s.msnu;	
  ...
  cout <<" " << mu << " " 
    ...
    displayGaugeCoupling(2) << " " <<
     calcRunningMt() << " " << endl;
  cout <<  displayPhys().mh0(1) << " " <<
    displayPhys().mA0(1) << " " <<
    displayPhys().mh0(2) << " " <<
    displayPhys().mHpm << " " <<
     displayPhys().thetaH << " " << endl;
     cout <<   displayPhys().msnu.display(3) << " " <<
    ...
    displayPhys().me.display(1, 1) << " " <<
     displayPhys().me.display(2, 1) << " " << endl;
  cout <<  displayPhys().thetat << " " <<
    displayPhys().thetab << " " <<
    ...
    displayPhys().mneut.display(2) << " " <<
     ...
     displayPhys().mneut.display(4) << " " << endl;
  cout << calcSinthdrbar() << " " <<
     displayTadpole1Ms() << " " <<
     displayTadpole2Ms() << " " << endl;
...
/// checked
void NmssmSoftsusy::charginos(int accuracy, double piwwtMS) {
...
double NmssmSoftsusy::thet(double a, double b, double c = 0.0) {
...
void NmssmSoftsusy::neutralinos(int accuracy, double piwwtMS, double pizztMS) {
  double tanb = displayTanb();
...
void NmssmSoftsusy::gluino(int accuracy) {
...
double NmssmSoftsusy::calcRunningMb() const {
  ...
  if (displayMu() != displayMz()) {
    ostringstream ii;
    ii << "NmssmSoftsusy::calcRunningMb called with mu=" <<
      displayMu() << endl; 
    throw ii.str();
  }
  
  NMdrBarPars forLoops(displayDrBarPars());
...
//PA:  for NMSSM modify mu to \mu_{eff} = \mu + \lambda * s / \sqrt{2} 
  if(NMSSM){
     double lam = displayLambda();
     double svev = displaySvev();
     mu = mu + lam * svev / root2;    
}
...
void NmssmSoftsusy::addSquarkCorrection(DoubleMatrix & mass) {
...
void NmssmSoftsusy::assignHiggs(DoubleVector & higgsm, DoubleVector & higgsc)
  const {
  NMdrBarPars f(displayDrBarPars());
...
void NmssmSoftsusy::assignHiggsSfermions(DoubleVector & higgsm, 
					DoubleVector & higgsc, 
					DoubleVector & dnu, 
					DoubleVector & dnd, 
					DoubleVector & cn, double beta) const {
...
/// Cecked 16.08.2005
void NmssmSoftsusy::addSbotCorrection(double p, 
...
void NmssmSoftsusy::doUpSquarks(double mt, double pizztMS, double sinthDRbarMS, 
			       int accuracy) { 
...
void NmssmSoftsusy::doDownSquarks(double mb, double pizztMS, double
...
void NmssmSoftsusy::doChargedSleptons(double mtau, double pizztMS, double
...
void NmssmSoftsusy::doSnu(double pizztMS, int accuracy) {
  double mSnuSquared;
...
void NmssmSoftsusy::treeSnu(double & mSnuSquared, double pizztMS, int family) {
...
extern double nmminimufb3(double lnH2) {

  /// Save initial parameters
  double initialMu = tempSoft1->displayMu();
  DoubleVector savePars(tempSoft1->display());

  NmssmSoftsusy temp(*tempSoft1);
...
double ufb3fn(double mu, double htau, double h2, int family, const NmssmSoftsusy
	      & temp) { 
  double vufb3 = 0.0;
...
double NmssmSoftsusy::ufb3sl(double mx) {
  ...
  /// minimufb3
  double Vmin = findMinimum(ax, bx, cx, nmminimufb3, tol, &lnx);
...
/// From hep-ph/9606211
double NmssmSoftsusy::qcdSusythresh(double alphasMSbar, double q) const {
  NMdrBarPars tree(displayDrBarPars());
  double mt = tree.mt;
...
double NmssmSoftsusy::qedSusythresh(double alphaEm, double q) const {
...
int NmssmSoftsusy::lsp(double & mass, int & posi, int & posj) const {
  int temp = 0, pos1 = 0, pos2 = 0;
  nmsPhysical s(displayPhys());
...
double NmssmSoftsusy::maxMass() const {
  int pos1, pos2;
  nmsPhysical s(displayPhys());
...
double NmssmSoftsusy::calcMs() const {
...
(void (*boundaryCondition)(NmssmSoftsusy &, const DoubleVector &),
 double mxGuess, 
 const DoubleVector & pars, const DoubleVector nmpars, int sgnMu, double tanb, const QedQcd &
 oneset, bool gaugeUnification, bool ewsbBCscale) {
  ...
  try {
    ...
    runto(mz);
    if (susyRpvBCatMSUSY) sparticleThresholdCorrections(tanb, pars);
...
double sumTol(const NmssmSoftsusy & in, const NmssmSoftsusy & out) {
  ...
  NMdrBarPars inforLoops(in.displayDrBarPars()), 
...
double NmssmSoftsusy::calcSinthdrbar() const {
...
double NmssmSoftsusy::getVev(double pizzt) const {
...
double NmssmSoftsusy::getVev() const {
...
void NmssmSoftsusy::sparticleThresholdCorrections(double tb, const DoubleVector & pars) {
  double mz = displayMz();  if (displayMu() != mz) {
    ostringstream ii;
    ii << "Called NmssmSoftsusy::sparticleThresholdCorrections "
...
double NmssmSoftsusy::displayMzRun() const { 
...
void NmssmSoftsusy::calcDrBarPars() {
  NMdrBarPars eg(displayDrBarPars());
  ...
  double mw2 = sqr(mw);
  //PA: new nmssm parameters for chargino mass matrix
  double lam = displayLambda();
  double svev = displaySvev();
  ...
  cout << " before mCh(2, 2) = "  << mCh(2, 2) << endl;
  if(NMSSM) mCh(2, 2) += lam * svev / root2;
  eg.mch = mCh.asy2by2(eg.thetaL, eg.thetaR);
  cout << " after mCh(2, 2) = "  << mCh(2, 2) << endl; 
 ...
 if(NMSSM) mNeut(3, 4) += - lam * svev / root2; 
 //PA: Singlino-Higgsino terms.  
  mNeut(3, 5) = - lam * vev * sin(beta) / root2; 
  mNeut(4, 5) = - lam * vev * cos(beta) / root2; 
  //PA: diagonal singlino mass terms
  mNeut(5, 5) = 2.0 * kap * svev + mus;
  mNeut.symmetrise();
  cout << " mNeut = "  << mNeut << endl;
  ...
  cout << "mixNeut = " << eg.mixNeut << endl;
  cout << "eg.mneut = "  << eg.mneut << endl; 
  //PA: in nmssm we have a 2x2 pseudoscalar mass matrix
  // (the third degree of freedom is a goldstone boson)
  DoubleMatrix MHP(2,2);
  ...
  if (Sdecoupled){
     ...
     // cout << "MHP(1,1) = " << MHP(1,1) << endl;
     MHP(1, 2) =  (alam - root2 * lam * kap * svev - lam * mus) * vev / root2; 
     MHP(2, 1) = MHP(1, 2);
     ...
     if(svev == 0) {
        flagszero();
        if(PRINTOUT > 1) cout << "non singlet-decupled NMSSM Higgs masses called hen s is zero"  << endl;
     }
     MHP(1, 1) = m3sqeff / (sin(beta) * cos(beta)); 
     MHP(1, 2) =  (alam - root2 * lam * kap * svev - lam * mus) * vev / root2; 
     MHP(2, 1) = MHP(1, 2);
     MHP(2, 2) =  (alam + 4.0 * lam * kap * svev / root2 
  ...
  cout << "mSq for MHP = " << mSq << endl;
  cout << "eg.thetaHP = "  << eg.thetaHP << endl;

  DoubleVector MA(mSq.apply(ccbSqrt));
  cout << "MA(1) = "  << MA(1) << endl;
  cout << "MA(2) = "  << MA(2) << endl;
  cout << "thetaHP = "  << eg.thetaHP << endl;
  if (MA(2) > MA(1)) eg.thetaHP = eg.thetaHP + PI * 0.5; 
  
  double mAsq = displayM3Squared() / (sin(beta) * cos(beta)); 
  double mAsqeff = m3sqeff / (sin(beta) * cos(beta));
  cout << "mAsq = " << mAsq << endl;
  cout << "mAsqeff = "  << mAsqeff << endl;
  cout << "mA = " << sqrt(mAsq) << endl;
  cout << "mAeff = "  << sqrt(mAsqeff) << endl;
  eg.mw = mw;
  eg.mz = mz;
  eg.mpzNeutralinos();
  eg.mpzCharginos();
  ...
  //PA: the CP even Higgs mass is 3x3 in the NMSSM with significant modifications to all terms, compared to the MSSM.  Taken from arxiv:0910.1785 but with non-zer \mu.
  DoubleMatrix mH(3, 3); 
  ...
  if (Sdecoupled){
     ...
     mH(2, 1) = mH(1 ,2);
     mH(3, 3) =  svev * (akap + 4.0 * sqr(kap) * svev + 3.0 * kap * mus);
     mH(1, 3) = root2 * lam * mueff * vev * cos(beta) 
        - (alam + root2 * lam * kap * svev + lam * mus) * vev * sin(beta) / root2;
     mH(3, 1) = mH(1, 3); 
     mH(2, 3) = root2 * lam * mueff * vev * sin(beta) 
        - (alam + root2 * lam * kap * svev + lam * mus) * vev * cos(beta) / root2;
     mH(3, 2) = mH(2, 3); 
}
  else{ 
     if(svev == 0) {
  ...
  //PA: code for testing, Z_3 case from hep-ph/0304049
  DoubleMatrix MHtest(3, 3);
  MHtest(1, 1) = svev / (2.0 * sin(beta) * cos(beta)) * (alam *root2 + lam * kap * svev) + (mz2 - 0.5 * sqr(lam * vev)) * 4.0 * sqr(sin(beta) * cos(beta));
  MHtest(1, 2) = - 0.5 * (mz2 - 0.5 * sqr(lam * vev)) * 4.0 * sin(beta) * cos(beta) * (sqr(cos(beta)) - sqr(sin(beta)));
  MHtest(1, 3) = - 0.5 * (svev / (2.0 * sin(beta) * cos(beta)) * (alam *root2 + lam * kap * svev) * 2.0 * sin(beta) * cos(beta) + lam * kap * sqr(svev) )* vev / svev * (sqr(cos(beta)) - sqr(sin(beta)));
  MHtest(2, 2) = mz2 * sqr( sqr(cos(beta)) - sqr(sin(beta))) +  0.5 * sqr(lam * vev) *  4.0 * sqr(sin(beta) * cos(beta));
MHtest(2, 3) = 0.5 * (2.0 * sqr(lam * svev) - svev / (2.0 * sin(beta) * cos(beta)) * (alam * root2 + kap * svev) * 4.0 * sqr(sin(beta) * cos(beta)) - lam * kap * sqr(svev) * 2.0 * sin(beta) * cos(beta)) * vev / svev;
MHtest(3, 3) = svev / (2.0 * sin(beta) * cos(beta)) * (alam * root2 + lam * kap * svev) * sin(beta) * cos(beta) * sqr(vev / svev) + 2.0 * sqr(kap) * sqr(svev) + svev * akap / root2 - 0.5 * lam * kap * sqr(vev) * sin(beta) * cos(beta);
 MHtest.symmetrise();

 cout << "MHtest = "  << MHtest << endl;
 DoubleMatrix mixHtest(3,3); DoubleVector mhtest(3);
 if (MHtest.diagonaliseSym(mixHtest, mhtest) > TOLERANCE *
     1.0e-3) { cout << "In warning"  << endl; 
    ostringstream ii;
    ii << "accuracy bad in neutralino diagonalisation"<< flush;
    throw ii.str(); 
 }
 cout << "mixHtest = "  << mixHtest << endl;
 cout << "mhtest = "  << mhtest << endl;
  DoubleMatrix mHmssm(2,2);
  mHmssm(1, 1) = mAsq * sqr(sin(beta)) + mz2 * sqr(cos(beta));
  mHmssm(1, 2) = - sin(beta) * cos(beta) * (mAsq + mz2); 
  mHmssm(2, 2) = mAsq * sqr(cos(beta)) + mz2 * sqr(sin(beta)); 
  mHmssm(2, 1) = mHmssm(1 ,2);
  cout << "mHmssm = "  << mHmssm <<endl;
  mSq = mHmssm.sym2by2(eg.thetaH);
   cout << "mSq = "  << mSq << endl;
  if (mSq(1) < 0. || mSq(2) < 0.) {
      flagTachyon(h0);
    if (PRINTOUT > 1) cout << " mH/h tachyon ";
  ...
  cout << "thetaH = "  << eg.thetaH << endl;

  if (mH.diagonaliseSym(eg.mixHiggs, eg.mhiggs) > TOLERANCE *
      1.0e-3) { cout << "In warning"  << endl; 
    ostringstream ii;
    ii << "accuracy bad in neutralino diagonalisation"<< flush;
    throw ii.str(); 
  ...
  cout << "eg.mixHiggs = "  << eg.mixHiggs << endl;
  cout << "eg.mhiggs = "  << eg.mhiggs << endl;
  int pos;
  double mh0 = sqrt(eg.mhiggs(1)); double mH1 = sqrt(eg.mhiggs(2)); double mH2 = sqrt(eg.mhiggs(3)); 
  cout << "mh0 = " << mh0 << endl;
  cout << "mH1 = "  << mH1 << endl;
 cout << "mH2 = "  << mH2 << endl;
  
  massorder(mh0, mH1, mH2);
  cout << "mh0 = " << mh0 << endl;
  cout << "mH1 = "  << mH1 << endl;
 cout << "mH2 = "  << mH2 << endl;
  eg.mh0(1) = mh0; eg.mH1 = mH1; eg.mH2 = mH2;                                            
  cout << "eg.mh0(1) = " << eg.mh0(1) << endl;
  cout << "eg.mH1 = "  << eg.mH1 << endl;
 cout << "eg.mH2 = "  << eg.mH2 << endl;
 eg.mA0(1) = sqrt(mAsq); eg.mHpm = sqrt(mAsqeff + mw2 - 0.5 *sqr(lam) * sqr(vev));
  

cout << "eg.mA0(1) = "  << eg.mA0(1) << endl;
  cout << "eg.mHpm = "  << eg.mHpm << endl;
  //PA: nmssm psuedoscalar masses.
  eg.mA1 = MA.min(pos); eg.mA2 = MA.max();
 
 cout << "eg.mA1 = "  << eg.mA1 << endl;
cout << "eg.mA2 = "  << eg.mA2 << endl;
...
double NmssmSoftsusy::piAA(double p, double q) const {/// checked 30.07.03
  NMdrBarPars tree(displayDrBarPars());
...
double NmssmSoftsusy::piZGT(double p, double q) const { ///! checked 7/6/6
  NMdrBarPars tree(displayDrBarPars());
  double alphaMsbar = dataSet.displayAlpha(ALPHA);
  double alphaDrbar = qedSusythresh(alphaMsbar, displayMu());
...
double nmrho2(double r) {/// checked 
  if (r <= 1.9)
...
double NmssmSoftsusy::sinSqThetaEff() {
  if (displayMu() != MZ) {
    throw("Should call NmssmSoftsusy::sinSqThetaEff() at MZ only\n");
  }
  double kl = 0.;
...
double NmssmSoftsusy::deltaVb(double outrho, double outsin, 
...
double NmssmSoftsusy::dRho(double outrho, double outsin, double alphaDRbar, 
...
double NmssmSoftsusy::dR(double outrho, double outsin, double alphaDRbar,
  ...
  double deltaR2LoopSm = alphaDRbar * sqr(displayGaugeCoupling(3)) / 
    ...
    nmrho2(tree.mh0(1) / mt) * (1.0 - deltaR) * outrho / 3.0;
...
void NmssmSoftsusy::rhohat(double & outrho, double & outsin, double alphaDRbar,
  ...
  if (displayMu() != mz) {
    ostringstream ii;   
    ii << "Called NmssmSoftsusy::rhohat "
       << "with scale" << displayMu() << endl;
    throw ii.str();
  }
...
void NmssmSoftsusy::methodBoundaryCondition(const DoubleVector & pars) {
  ostringstream ii;
  ii << "Should only use NmssmSoftsusy::methodBoundaryCondition in derived"
     << " objects.\n";
  throw ii.str();
}

void NmssmSoftsusy::rpvSet(const DoubleVector & parameters){
  ostringstream ii;
  ii << "Should only use NmssmSoftsusy::rpvSet in derived"
     << " objects.\n";
  throw ii.str();
}
...
void NmssmSoftsusy::isajetNumbers764 
...
void NmssmSoftsusy::ssrunInterface764(const char fname [80], 
...
void NmssmSoftsusy::ssrunInterface764Inside(const char fname [80], 
					   fstream & softOutput) 
  const { 
...
void NmssmSoftsusy::isawigInterface764(const char herwigInputFile [80], 
...
void NmssmSoftsusy::isajetInterface764(const char fname[80]) const {
...
void NmssmSoftsusy::spinfoSLHA(ostream & out) {
  ...
  if (displayProblem().testSeriouNMsProblem()) 
    out << "     4   Point invalid: " << displayProblem() << endl;
}

void NmssmSoftsusy::softsusySLHA(ostream & out, double mgut) {
...
void NmssmSoftsusy::massSLHA(ostream & out) {
  nmsPhysical s(displayPhys());
...
void NmssmSoftsusy::sfermionsSLHA(ostream & out) {
  nmsPhysical s(displayPhys());
...
void NmssmSoftsusy::alphaSLHA(ostream & out) {
  out << "Block alpha                   " << 
...
void NmssmSoftsusy::inomixingSLHA(ostream & out) {
  nmsPhysical s(displayPhys());
...
void NmssmSoftsusy::sfermionmixSLHA(ostream & out) {
    nmsPhysical s(displayPhys());
    DoubleMatrix m(2, 2);
...
void NmssmSoftsusy::gaugeSLHA(ostream & out) {
  double gp = displayGaugeCoupling(1) * sqrt(0.6);
...
void NmssmSoftsusy::yukawasSLHA(ostream & out) {
...
void NmssmSoftsusy::hmixSLHA(ostream & out) {
...
void NmssmSoftsusy::msoftSLHA(ostream & out) {
...
void NmssmSoftsusy::drbarSLHA(ostream & out, int numPoints, double qMax) {
    int n = 0;
...
void NmssmSoftsusy::sminputsSLHA(ostream & out) {
...
void NmssmSoftsusy::slha1(ostream & out, const char model[], 
					  const DoubleVector & pars, 
...
void extendedSugraBcs(NmssmSoftsusy & m, const DoubleVector & inputParameters) {
  int i;
...
/// conditions. 
void sugraBcs(NmssmSoftsusy & m, const DoubleVector & inputParameters) {
...
void nonUniGauginos(NmssmSoftsusy & m, const DoubleVector & inputParameters) {
  double m0 = inputParameters.display(1);
  double m12 = inputParameters.display(2);
  double a0 = inputParameters.display(3);
...
void lvsBcs(NmssmSoftsusy & m, const DoubleVector & inputParameters) {
  double m0  = inputParameters.display(1);
...
void gmsbBcs(NmssmSoftsusy & m, const DoubleVector & inputParameters) {
...
void userDefinedBcs(NmssmSoftsusy & m, const DoubleVector & inputParameters) {
...
/// 0 is neutralino posi = #, posj = 0
int NmssmSoftsusy::nlsp(double & mass, int & posi, int & posj) const {
...
bool testLEPHiggs(const NmssmSoftsusy & r, double error) {
...
/// checked 22/04/06
double NmssmSoftsusy::smPredictionMW() const {
...
double NmssmSoftsusy::twoLoopGm2(double amu1Loop) const {
...
/** Start of NmssmSoftsusyAltEwsb object **/
void NmssmSoftsusyAltEwsb::rewsb(int sgnMu, double mt, double muOld) {
  ...
  double newMh1sq, newMh2sq;
    ...
    double m3sqnew = 0., kapnew = 0.;
    //PA:  again using a rewsbM3sq hich can work for Z3 symmetric version
    //with kappa as output
    //but nonetheless setting Z3=false and not using this yet  
    if (rewsbM3sq(displaySusyMu(), m3sqnew, kapnew, false) == 0) 
       {flagM3sq(false);}
...
void NmssmSoftsusyAltEwsb::setEwsbConditions(const DoubleVector & inputs) {
...
void NmssmSoftsusy::doQuarkMixing(DoubleMatrix & mDon, 
				 DoubleMatrix & mUpq) {
...
/// Again, another dummy - useful in alternative EWSB conditions sometimes
void NmssmSoftsusy::setEwsbConditions(const DoubleVector & inputs) {
  return; 
}
...
// electroweak scale.
void splitGmsb(NmssmSoftsusy & m, const DoubleVector & inputParameters) {
