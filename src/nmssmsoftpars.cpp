...
namespace softsusy {
  ...
  /// thresholds.
  /// CHECKED: 24/05/02
  SoftParsNmssm SoftParsNmssm::beta2(nmsBrevity & a) const {
    /// Constants for gauge running
    static DoubleVector clBeta(3);
    if (clBeta(1) < 1.0e-5) /// Constants not set yet
      setBetaLambda(clBeta);
    
    NmssmSusy bnms(NmssmSusy::beta(a));
    MssmSoftPars cnms(beta2());
    SoftParsNmssm base(bnms, cnms);
     ...
     const double &smu = displayNmssmSusy().displaySusyMu();
     ...
     if (displayNmssmLoops() > 0) {
     dmG = base.displayGaugino();
     
     dm3sq = base.displayM3Squared();
     
     dm3sq += (4.0 * NmssmSusy::displaySusyMu() * lam * hlam
  ...
  void SoftParsNmssm::addAmsb(double maux) {
    MssmSoftPars::addAmsb(displayMssmSusy(), maux);
    //    SoftPars<NmssmSusy,nmsBrevity>::addAmsb(maux);
     NmssmSusy Nms(displayNmssmSusy());
  ...
  void SoftParsNmssm::universalTrilinears(double a0) {
    SoftParsNmssm::universalTrilinears(a0);
