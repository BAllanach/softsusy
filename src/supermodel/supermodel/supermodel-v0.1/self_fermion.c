            ...
            TSIL_COMPLEX *VSffFS, TSIL_COMPLEX *VSfffS) 
{
  ...
  if (0 == swapmode) {
    ...
    y = input_data2->z;
    u = input_data1->u;
    mustbe0 = TSIL_FABS (qq - input_data2->qq) 
            + TSIL_FABS (x - input_data2->x) 
            + TSIL_FABS (u - input_data2->u) 
            + TSIL_FABS (v - input_data2->v) 
            + TSIL_FABS (s - input_data2->s);
  } else {
    x = input_data1->u;
    z = input_data1->y;
    y = input_data2->y;
    u = input_data1->x;
    mustbe0 = TSIL_FABS (qq - input_data2->qq) 
            + TSIL_FABS (x - input_data2->u) 
            + TSIL_FABS (u - input_data2->x) 
            + TSIL_FABS (v - input_data2->v) 
            + TSIL_FABS (s - input_data2->s);
  }

  if (mustbe0 > 1000.0 * TSIL_TOL)
