...
int SUMO_Find_b_and_mu (int loop_order, SUMO_COMPLEX phasemu)
{
  SUMO_REAL sin2beta;
  SUMO_REAL mmu, mmd, absmu2;
  SUMO_REAL temp1, temp2, temp3, temp4, temp5;
  int j,k;
  int iterations = 5; /* Should be good enough. */
  int error_code = 0;

  SUMO_Update ();
  temp1 = -0.5L * (g2plusgp2) * (vuvd);
  temp2 = (vu * vd)/(vu2 - vd2);
  temp3 = -0.25L * (g2plusgp2) * (v2);
  temp4 = (vd2)/(vu2 - vd2);
  temp5 = -(vu2)/(vu2 - vd2);

  /* printf("In Find_b_and_mu; starting values are:\n"); */
  /* printf("b = %.6f\n",(double) TSIL_CREAL(b)); */
  /* printf("mu = %.6f\n",(double) TSIL_CREAL(mu)); */
  ...
  for (j=0; j<iterations; j++) {
    ...
    for (k=1; k<=loop_order; k++) {
      SUMO_V_firstderivs_numerical (k, 0.01L, 0.1L);
      mmu += dVdvu[k]/(2.0L * vu);
      mmd += dVdvd[k]/(2.0L * vd);
    }

    b = temp1 + temp2 * (mmd - mmu);
    absmu2 = temp3 + temp4 * mmd + temp5 * mmu;
    mu = phasemu * TSIL_CSQRT(absmu2);
  ...
  /* Things that could go wrong with a particular model, which must be
     noted but shouldn't be allowed to stop the program: */
  if (SUMO_CREAL(b) < 0) error_code += 1;
  if (absmu2 < 0) error_code += 2;
