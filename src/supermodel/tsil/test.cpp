...
{
  int i;
  TSIL_COMPLEXCPP res, dat[6], bolddat[6][3];
  TSIL_DATA result;
  TSIL_REAL x, y, z, u, v, s, qq;
  clock_t t0, t1;

  if (argc != 8)
    TSIL_Error_ ("main", "Incorrect number of args; should be 7", 1);
  ...
  res = TSIL_GetFunction_ (&result, "Txvu");
  cout << "result is " << res << endl;

  TSIL_GetData_ (&result, "T", dat);
  for (i=0; i<6; i++) cout << "T[" << i << "] = " << dat[i] << endl;

  TSIL_GetBoldData_ (&result, "T", bolddat);
  for (i=0; i<6; i++) cout << "T[" << i << "][1] = " << bolddat[i][1] << endl;

  TSIL_Vanalytic_ (x, y, 0.0L, y, s + 0.1L*I, qq, &res);
  cout << "result is " << res << endl;

  TSIL_Manalytic_ (0.0L, x, 0.0L, x, x, s, &res);
  cout << "result is " << res << endl;
