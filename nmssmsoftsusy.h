...
class NmssmSoftsusy: public SoftParsNmssm {
  /// Includes
  /// - Soft terms
  /// - DRbar masses
  /// - RGEs
...
private:
  nmsPhysical physpars; ///< Contains pole masses and mixings of sparticles
  NMdrBarPars forLoops; ///< Contains DRbar tree-level masses
  NMsProblem problem;   ///< Contains problem flags 
...
public:
  ...
  /// parameter. 
  NmssmSoftsusy(const SoftParsNmssm & s, const nmsPhysical & sp, double mu, int l, 
	       int t, double hv);
  ...
  inline const NmssmSoftsusy & displayMssmSoft() const;

  /// Displays physical parameters only
  inline const nmsPhysical & displayPhys() const;

  /// Displays tree-level masses and mixings of sparticles and third
  /// generation fermions
  inline const NMdrBarPars & displayDrBarPars() const;
  /// Returns any problem flags associated with the object
  const NMsProblem & displayProblem() const {return problem; };
  ...
  const NmssmSoftsusy & displaySoftsusy() const { return *this; }
  ...
  void flagszero(){problem.szero = true;};
  ...
  void setSoftsusy(const NmssmSoftsusy & s) { *this = s; };
  ...
  void setPhys(const nmsPhysical & s) { physpars = s; };
  /// Sets tree-level DRbar parameters
  void setDrBarPars(const NMdrBarPars & s) { cout << "using nmssm version setDrBarPars"  << endl;
     cout << "setting forloops :" << forLoops  << endl;
cout << "to "  << s << endl;
forLoops = s; };
  ...
  */
  ...
  /// minimum 
  virtual int rewsbMu(int sgnMu, double & mu, double & s, bool Z3) const;
  ...
  double lowOrg(void (*boundaryCondition)
		...
		double mxGuess, 
		const DoubleVector & pars, const DoubleVector nmpars, int sgnMu, double tanb,
  ...
  /// The idea is that NmssmSoftsusy objects are UNmixed. Therefore this method
...
inline NmssmSoftsusy::NmssmSoftsusy()
  : SoftParsNmssm(), physpars(), forLoops(), 
...
inline NmssmSoftsusy::NmssmSoftsusy(const NmssmSoftsusy & s)
  : SoftParsNmssm(s.displaySoftPars()), physpars(s.displayPhys()), 
...
inline NmssmSoftsusy::NmssmSoftsusy
(const SoftParsNmssm & s, const nmsPhysical & sp, double mu, int l, int t, 
 double hv) 
  : SoftParsNmssm(s), physpars(sp), forLoops(), problem(), msusy(0.0),
...
inline const NmssmSoftsusy & NmssmSoftsusy::displayMssmSoft() const { return *this; }

inline const QedQcd & NmssmSoftsusy::displayDataSet() const { return dataSet; }

inline const nmsPhysical & NmssmSoftsusy::displayPhys() const { return physpars; }

inline const NMdrBarPars & NmssmSoftsusy::displayDrBarPars() const { 
  return forLoops; 
}

inline double NmssmSoftsusy::displayMinpot() const { return minV; } 
inline double NmssmSoftsusy::displayMsusy() const { return msusy; } 
inline double NmssmSoftsusy::displayMw() const { return mw; } 

inline double NmssmSoftsusy::displayTadpole1Ms() const {
  return physpars.t1OV1Ms; 
}

inline double NmssmSoftsusy::displayTadpole2Ms() const {
  return physpars.t2OV2Ms; 
}

inline void NmssmSoftsusy::setMinpot(double f) { minV = f; }
inline void NmssmSoftsusy::setMsusy(double f) { msusy = f; }
inline void NmssmSoftsusy::setMw(double f) { mw = f; }
inline void NmssmSoftsusy::doUfb3(double mgut) { setMinpot(ufb3sl(mgut) -
							  realMinMs()); } 
...
/// Formatted output
ostream & operator <<(ostream &, const NmssmSoftsusy &); 
...
//PA: universal msugra bcs in the mssm limit of the general mssm
...
bool testLEPHiggs(const NmssmSoftsusy & r, double error = 3.0);
...
class NmssmSoftsusyAltEwsb: public NmssmSoftsusy {
private:
...
void extendedSugraBcs2(NmssmSoftsusy & m, 
